...
exports.registrationFirst = async (req, res) => {
    try {
        const { fullName, dob, registrationNumber, doctorCategoryId, fee } = req.body;
        const user = await User.findById(req.params.id);
        if (!user) {
            return res.status(404).send({ message: "not found" });
        }
        const data = await doctorCategory.findById({ _id: doctorCategoryId });
        if (!data || data.length === 0) {
            return res.status(400).send({ msg: "not found" });
        }
        user.doctorCategoryId = data._id;
        user.completeProfile = true;
        user.fullName = fullName || user.fullName;
        user.dob = dob || user.dob;
        user.fee = fee || user.fee;
        user.registrationNumber = registrationNumber || user.registrationNumber;
        const updated = await user.save();
        res.status(200).send({ message: "updated", userId: updated._id, complete: updated.completeProfile, });
    } catch (err) {
        console.log(err);
        res.status(500).send({
