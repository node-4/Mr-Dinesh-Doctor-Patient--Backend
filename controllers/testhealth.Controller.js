...
exports.editTestProduct = async (req, res) => {
    try {
        ...
        if (updateid) {
            ...
            if (categorydetails) {
                res.status(200).json({ message: "Sub Category updated successfuly", code: 200, data: categorydetails })
            } else {
                res.status(501).json({
...
exports.deletetesthealth = async (req, res) => {
    try {
        ...
        if (testID) {
            res.status(200).json({ message: "Health Test Deleted Successfully" })
        } else {
            res.status(404).json({ message: "Invalid Health Test ID" })
        }
...
exports.gethealthtestbyheathcategoryid = async (req, res) => {
    ...
    try {
        ...
        if (singleHealth) {
            res.status(200).json({ message: "success", count: singleHealth.length, data: singleHealth })
        } else {
            res.status(404).json({ status: "Failed", message: "Invalid Health Test ID" })
        }
...
exports.getallhealthtest = async (req, res) => {
    try {
        const data = await Testhealth.find().populate({ path: "healthtestcategoryid" })
        if (data.length>0) {
            res.status(200).json({message: "success",count: data.length,data: data});
        } else {
